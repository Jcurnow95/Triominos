...
import { Room, createRoom, createSoloRoom, getRoom, isBot, joinRoom, markDisconnected, rejoinRoom } from './rooms.js';
...
function broadcastLobby(io: IOServer, room: Room): void {
  io.to(room.code).emit('lobbyUpdate', toLobbyState(room));
}

function broadcastGame(io: IOServer, room: Room, event: 'gameStarted' | 'gameUpdate'): void {
  if (!room.game) return;
...
function botThinkMs(room: Room): number {
 ...
 */
async function runBotTurns(io: IOServer, room: Room): Promise<void> {
  ...
  try {
    ...
    for (let guard = 0; guard < 500; guard++) {
      ...
      if (round.phase === 'round-ended') return;
      ...
      if (room.game !== game) return; // room was reset underneath us
...
export function registerSocketHandlers(io: IOServer, socket: IOSocket): void {
  ...
  socket.on('disconnect', () => {
    ...
    if (!result) return;
    broadcastLobby(io, result.room);
    if (result.room.game) broadcastGame(io, result.room, 'gameUpdate');
  });
}
