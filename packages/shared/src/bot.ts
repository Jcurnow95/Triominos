import { Board, Placement, boardTiles, evaluateBonus, findLegalPlacements, placeTile } from './board.js';
import { CellCoord } from './grid.js';
import { GameState, currentPlayerId, mustPassInsteadOfDrawing } from './gameState.js';
import { scorePlay, scoreStartingTile } from './scoring.js';
import { Tile, generateDeck, tileSum } from './tiles.js';

export type BotDifficulty = 'easy' | 'normal' | 'hard';

export type BotAction =
  | { type: 'place'; tileId: string; cell: CellCoord }
  | { type: 'draw' }
  | { type: 'pass' }
  | { type: 'choose-starter'; tileId: string };
...
function bestOpponentReply(board: Board, unseen: Tile[]): number {
  let best = 0;
...
function collectMoves(hand: Tile[], board: Board): ScoredMove[] {
...
export function chooseBotAction(
  ...
  if (!player) throw new Error('Bot is not a player in this game');

  if (round.phase === 'awaiting-starter-choice') {
    if (round.starterChoice?.playerId !== playerId) throw new Error('Not the bot\'s starter choice');
  ...
  if (currentPlayerId(round) !== playerId) throw new Error('Not the bot\'s turn');
