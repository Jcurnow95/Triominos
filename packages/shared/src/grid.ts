...
export function parseCellKey(key: string): CellCoord {
  ...
  return { q: Number(q), r: Number(r), orient: orient as Orient };
}
...
export function cellVertices(c: CellCoord): [VertexCoord, VertexCoord, VertexCoord] {
  const { q, r, orient } = c;
  if (orient === 'up') {
...
export function edgeNeighbors(c: CellCoord): CellCoord[] {
  const { q, r, orient } = c;
  if (orient === 'up') {
    return [
      { q, r, orient: 'down' },
 ...
 */
export function ringAround(v: VertexCoord): { cell: CellCoord; vertexIndex: number }[] {
  const { q, r } = v;
  return [
...
export function cellCentroid(c: CellCoord): { x: number; y: number } {
...
export function vertexPoint(v: VertexCoord): { x: number; y: number } {
