...
export function isFreestyle(tile) {
 ...
 */
export function tileLabel(values) {
 ...
 */
export function generateFreestyleTiles(count) {
    if (count <= 0)
        return [];
 ...
 */
export function generateDeck(setCount = 1, freestyleCount = 0) {
    const singleSet = [];
    for (let a = 0; a <= 5; a++) {
        for (let b = a; b <= 5; b++) {
            for (let c = b; c <= 5; c++) {
